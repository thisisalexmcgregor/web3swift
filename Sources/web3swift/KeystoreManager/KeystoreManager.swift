...
public class KeystoreManager: AbstractKeystore {
    ...
    public static func managerForPath(_ path: String, scanForHDwallets: Bool = false, suffix: String? = nil) -> KeystoreManager? {
        guard let newManager = ((try? KeystoreManager(path, scanForHDwallets: scanForHDwallets, suffix: suffix)) as KeystoreManager??), let manager = newManager else { return nil }
        return manager
    }
